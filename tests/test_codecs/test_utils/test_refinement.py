...
class TestRefineKeypointsDarkUDP(TestCase):
    ...
    def test_flat_heatmap_keeps_keypoints_unchanged(self):
        ...
        refined = refine_keypoints_dark_udp(
            keypoints.copy(), heatmaps.copy(), blur_kernel_size=11
        )
    ...
    def test_gaussian_heatmap_moves_towards_true_peak(self):
        ...
        self.assertGreater(refined_xy[0], initial_xy[0])
        self.assertLess(refined_xy[1], initial_xy[1])
        self.assertLess(
            np.linalg.norm(refined_xy - true_peak),  # euclidean distance to true peak is less
            np.linalg.norm(initial_xy - true_peak),
        )
